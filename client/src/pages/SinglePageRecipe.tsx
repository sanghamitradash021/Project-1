...
//    * If no ID is provided, an error is displayed.
//    */
//   useEffect(() => {
//     if (!id) {
//       setError('No recipe ID found in the URL');
...
//   /**
//    * Converts ingredients string into an array if it is a string.
//    */
//   const ingredientsArray = Array.isArray(recipe.ingredients)
//     ? recipe.ingredients
//     : typeof recipe.ingredients === 'string'
...
const SingleRecipe: React.FC = () => {
  ...
  const handleDelete = async () => {
    ...
    if (confirmed) {
      try {
        ...
        console.error('Error deleting recipe:', error);
        toast.error(ERROR_DELETING_RECIPE); // Show error toast
      }
    }
  };
  ...
  /**
   * Function to refresh the recipe after it is updated
   */
  const refreshRecipe = async () => {
    try {
      const response = await axios.get(`${API_URL}${id}`);
      setRecipe(response.data); // Set the updated recipe details
    } catch (error) {
      console.error('Error fetching updated recipe:', error);
      toast.error(TOAST_ERROR_MESSAGE); // Show error toast if refresh fails
    }
  };
